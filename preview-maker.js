...
const get = (node, className, tag) => Array.from(node.getElementsByTagName(tag)).filter(
    (element) => element.getAttribute && element.getAttribute(`class`) === className
);
...
const makePrevDesktop = async (themeBuffer) => {
    ...
    for (const background in variablesList) {
        for (const elementName in variablesList[background]) {
      ...
      theme[avatarVariable] || defaultVariablesValues[avatarVariable];
        ...
        if (avatarAndBackgroundDifference < 25) {
            ...
            for (const avatarShadow of getElementsByClassName(
                ...
                const hslChoose = rgbToHsl(avatarTextColor);
